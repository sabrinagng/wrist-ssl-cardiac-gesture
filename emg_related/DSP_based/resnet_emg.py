...
def parse_stack(stack_str):
    """Parse comma-separated stack feature string.

    Examples: 'none' -> [], 'rms' -> ['rms'], 'rms,mav,wl' -> ['rms','mav','wl']
    """
...
def load_raw_windowed_data(data_dir, label_dir, subjects):
    ...
    for subj in subjects:
        ...
        if not os.path.exists(subj_data_dir):
            print(f"  [!] {subj} data dir not found, skipping")
            continue
...
def rep_based_split(rep_arr, test_frac=0.2, val_frac=0.15, seed=42):
    ...
    This prevents overlapping sliding windows from leaking across sets.
...
def generate_kfold_splits(rep_arr, n_folds=5, seed=42):
    ...
    for k in range(n_folds):
        ...
        # Make sure val rep is not in test
...
class StackFeatureLayer(nn.Module):
    ...
    Given raw EMG (B, C, T), computes sliding-window features (RMS, MAV, WL)
    and returns (B, C, n_features, T_down) aligned to the TF representation.

    These are appended to dim=2 (frequency axis) of the spectrogram/scalogram,
    so channels (dim=1) stay at 2.
    """
    ...
    def forward(self, raw_signal, target_time_len):
        """
        Args:
            raw_signal: (B, C, T) raw EMG
            target_time_len: int, time dim of TF output to match
        Returns:
            (B, C, n_features, target_time_len)
        """
        B, C, T = raw_signal.shape

        # Sliding window: (B, C, n_windows, window_size)
        ...
        for feat_name in self.stack_list:
            if feat_name == 'rms':
                ...
                feat = torch.mean(x_unfold ** 2, dim=-1)  # mean power (Parseval's)
            else:
                continue
            features.append(feat)  # each: (B, C, n_windows)

        # Stack: (B, C, n_features, n_windows)
        stack = torch.stack(features, dim=2)

        # Resize time axis to match TF output
        ...
        # Log-scale to match TF magnitude range
...
class LogMelTransform(nn.Module):
    def __init__(self, n_fft=256, hop_length=64, n_mels=128,
        ...
        self.hop_length = hop_length
        self.register_buffer('window', torch.hann_window(n_fft))
        mel_fb = self._build_mel_filterbank(n_mels, n_fft, fs, f_min, f_max)
    ...
    def _build_mel_filterbank(self, n_mels, n_fft, fs, f_min, f_max):
        n_freqs = n_fft // 2 + 1
        mel_points = np.linspace(self._hz_to_mel(f_min), self._hz_to_mel(f_max), n_mels + 2)
        hz_points = self._mel_to_hz(mel_points)
        bin_points = np.floor((n_fft + 1) * hz_points / fs).astype(np.int64)
...
class ResNet2dBranch(nn.Module):
    """2D Mini-ResNet branch with selectable TF front-end + optional stack features."""
    def __init__(self, in_signal_channels, transform_type='stft',
                 n_fft=256, hop_length=64, n_scales=128,
                 f_min=20.0, f_max=450.0, fs=2000, n_mels=128,
                 stack_list=None):
        super().__init__()
        ...
        self.conv1 = nn.Conv2d(in_signal_channels, 16, 7, stride=2, padding=3, bias=False)
        ...
        self.layer4 = self._make_layer(128, 1, 2)
    ...
    def _make_layer(self, out_ch, n, stride):
        ...
        if stride != 1 or self.in_channels_current != out_ch:
            ds = nn.Sequential(
                nn.Conv2d(self.in_channels_current, out_ch, 1, stride=stride, bias=False),
                nn.BatchNorm2d(out_ch))
        layers = [BasicBlock2d(self.in_channels_current, out_ch, stride, ds)]
        self.in_channels_current = out_ch
        for _ in range(1, n):
    ...
    def forward(self, x):
        # x: (B, 2, T_raw)
        tf_out = self.tf_transform(x)  # (B, 2, F, T_down)

        if self.stack_layer is not None:
            stack_out = self.stack_layer(x, tf_out.shape[-1])  # (B, 2, n_stack, T_down)
            tf_out = torch.cat([tf_out, stack_out], dim=2)     # (B, 2, F+n_stack, T_down)

        x = self.maxpool(F.relu(self.bn1(self.conv1(tf_out))))
        x = self.layer4(self.layer3(self.layer2(self.layer1(x))))
        return self.avgpool(x).flatten(1)


class EMGResNet2d(nn.Module):
    def __init__(self, emg_channels=2, num_classes=9, dropout=0.3,
        ...
        self.backbone = ResNet2dBranch(
            emg_channels, transform_type,
            n_fft, hop_length, n_scales, f_min, f_max, fs, n_mels,
...
def mixup_data(x, y, alpha=0.2):
    ...
    lam = max(lam, 1 - lam)  # ensure lam >= 0.5 so first sample dominates
    batch_size = x.size(0)
...
def mixup_criterion(criterion, logits, y_a, y_b, lam):
    """Mixup loss: weighted sum of losses for both targets."""
...
def plot_per_class_accuracy(y_true, y_pred, output_path, title="Per-Gesture Accuracy"):
...
def plot_intra_subject_summary(subject_results, output_path, title=""):
...
def save_training_log(log_dir, config, results):
    with open(os.path.join(log_dir, "training_log.json"), 'w') as f:
        json.dump({"timestamp": datetime.now().isoformat(),
                    "config": config, "results": results}, f, indent=2)
...
def run_cross_subject(
    ...
    log_dir = os.path.join(base_log_dir, f"{window_label}_{mode_str}_cross_{timestamp}")
    ...
    print(f"EMG: ({emg_time}, {emg_channels}), Classes: {num_classes}")
    ...
    try:
        sample = torch.randn(2, emg_channels, emg_time).to(device)
        with torch.no_grad():
            out = model(sample)
        print(f"Forward pass OK: {list(sample.shape)} -> {list(out.shape)}")
        del sample, out
    ...
    test_loss, test_acc, y_pred_shifted, y_true_shifted = evaluate(
        model, test_loader, criterion, device)

    y_pred = np.array([idx_to_label[i] for i in y_pred_shifted])
    y_true = np.array([idx_to_label[i] for i in y_true_shifted])
    ...
    per_gesture = {}
    for g in sorted(set(y_true)):
        mask = y_true == g
        acc = accuracy_score(y_true[mask], y_pred[mask]) * 100
        per_gesture[int(g)] = acc
        print(f"  {g} ({GESTURE_NAMES.get(g, '')}): {acc:.1f}%")

    print("\n" + classification_report(y_true, y_pred,
          target_names=[GESTURE_NAMES.get(g, f"G{g}") for g in sorted(set(y_true))]))
    ...
    plot_confusion_matrix(y_true, y_pred, os.path.join(log_dir, "confusion_matrix.png"),
                          title=f"{mode_str} Cross ({window_label}) - {test_acc*100:.1f}%")
    plot_per_class_accuracy(y_true, y_pred, os.path.join(log_dir, "per_gesture_accuracy.png"),
                            title=f"{mode_str} Per-Gesture ({window_label}) - {test_acc*100:.1f}%")
    ...
    save_training_log(log_dir, {
        ...
        "test_accuracy": test_acc * 100, "test_loss": test_loss,
        "best_val_accuracy": best_val_acc * 100,
        "per_subject": per_subject, "per_gesture": per_gesture,
    })
...
def run_intra_subject(mode, transform_type, window_label,
    ...
    use_kfold = n_folds > 1
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = os.path.join(base_log_dir, f"{window_label}_{mode_str}_intra_{timestamp}")
    os.makedirs(log_dir, exist_ok=True)
    ...
    if use_kfold:
        ...
        print(f"Split: Rep-based {(1-args.test_split)*100:.0f}% train / {args.test_split*100:.0f}% test")
    use_amp = device.type == 'cuda'
    print(f"Device: {device}, AMP: {use_amp}")
    if args.augment:
    ...
    subject_results = []
    total_params = 0
    global_y_true = []   # collect across all subjects for aggregated CM
    global_y_pred = []
    ...
    for subj in subjects:
        ...
        emg_channels = emg_list[0].shape[1]
        ...
        # ---- Single split (n_folds=1) or K-fold (n_folds>1) ----
        if use_kfold:
        ...
        for fold_i, (train_idx, val_idx, test_idx, train_reps, val_reps, test_reps) in enumerate(folds):
        ...
        if use_kfold:
            ...
            print(f"\n  >>> {subj} {n_folds}-FOLD CV: {overall_acc:.1f}% "
                  f"(fold mean: {mean_fold_acc:.1f}% ± {std_fold_acc:.1f}%, "
        ...
        per_gesture = {}
        for g in sorted(set(all_y_true)):
            mask = all_y_true == g
            per_gesture[int(g)] = accuracy_score(
                all_y_true[mask], all_y_pred[mask]) * 100

        # Plot confusion matrix
        if use_kfold:
            cm_title = f"{subj} {mode_str} ({window_label}) {n_folds}-Fold - {overall_acc:.1f}%"
        else:
            cm_title = f"{subj} {mode_str} ({window_label}) - {overall_acc:.1f}%"
        plot_confusion_matrix(
            all_y_true, all_y_pred,
            os.path.join(log_dir, f"confusion_matrix_{subj}.png"),
            title=cm_title)

        # Plot training curves for single-split mode
        if not use_kfold:
            plot_training_curves(train_losses_last, val_losses_last,
                                 train_accs_last, val_accs_last,
                                 os.path.join(log_dir, f"training_curves_{subj}.png"))
    ...
    print(f"\n{'='*70}")
    if use_kfold:
        print(f"INTRA-SUBJECT SUMMARY ({mode_str}, {window_label}, {n_folds}-Fold CV)")
        print(f"{'='*70}")
        ...
        print(f"INTRA-SUBJECT SUMMARY ({mode_str}, {window_label})")
        print(f"{'='*70}")
        print(f"{'Subject':<8} {'Train':>6} {'Val':>5} {'Test':>5} {'Test Acc':>10} {'Val Acc':>10}")
        print("-" * 52)
        for r in subject_results:
    ...
    plot_title = (f"{mode_str} Intra ({window_label}, {n_folds}-Fold CV)" if use_kfold
                  else f"{mode_str} Intra ({window_label})")
    plot_intra_subject_summary(
        subject_results,
        os.path.join(log_dir, "intra_subject_summary.png"),
        title=plot_title)
    ...
    if len(global_y_true) > 0:
        ...
        plot_confusion_matrix_pct(
            ...
            os.path.join(log_dir, "confusion_matrix_all_subjects.png"),
    ...
    save_training_log(log_dir, {
        ...
        "mixup_alpha": args.mixup_alpha if args.augment else 0,
...
def main():
    ...
    parser.add_argument("--log-dir", default=os.path.join(script_dir, "results_resnet18_emg"))
    ...
    parser.add_argument("--mixup-alpha", type=float, default=0.2,
                        help="Mixup alpha (0 to disable, default 0.2, only used with --augment)")
    parser.add_argument("--noise-std", type=float, default=0.05,
                        help="Max Gaussian noise std for augmentation (default 0.05)")
    parser.add_argument("--warmup-epochs", type=int, default=0,
                        help="Linear LR warmup epochs (0=off, try 5 to enable)")
    parser.add_argument("--n-folds", type=int, default=1,
                        help="Folds for intra-subject CV (1=single rep-split [default], 5=5-fold CV)")
    parser.add_argument("--scheduler", default="cosine",
                        choices=["cosine", "plateau"],
                        help="LR scheduler: cosine (CosineAnnealingWarmRestarts) or plateau (ReduceLROnPlateau)")
    parser.add_argument("--cosine-t0", type=int, default=15,
                        help="T_0 for CosineAnnealingWarmRestarts (default 15)")
    ...
    if args.eval_mode == "intra":
        all_subjects = [s.strip() for s in args.subjects.split(",")] if args.subjects \
                       else [f"S{i:02d}" for i in range(1, 21)]
    else:
        train_subjs = parse_range(args.train)
        test_subjs = parse_range(args.test)
